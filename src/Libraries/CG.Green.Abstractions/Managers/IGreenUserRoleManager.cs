...
public interface IGreenUserRoleManager
{
    ...
    /// <summary>
    /// This method searches for role names that are associated with the given user.
    /// </summary>
    ...
    /// <see cref="string"/> objects.</returns>
    /// <exception cref="ArgumentException">This exception is thrown whenever one
    /// or more arguments are missing, or invalid.</exception>
    /// <exception cref="ManagerException">This exception is thrown whenever the
    /// manager fails to complete the operation.</exception>
    Task<IEnumerable<string>> FindByUserIdAsync(
        string userId,
        CancellationToken cancellationToken = default
        );

    /// <summary>
    /// This method updates the roles for the given <see cref="GreenUser"/> object,
    /// in the underlying storage.
    /// </summary>
    /// <param name="greenUser">The user to use for the operation.</param>
    /// <param name="userRoles">The models to updated in the underlying storage.</param>
    /// <param name="userName">The user name of the person performing the 
    /// operation.</param>
    /// <param name="cancellationToken">A cancellation token that is monitored
    /// for the lifetime of the method.</param>
